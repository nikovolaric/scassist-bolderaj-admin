...
import { getOneArticle, updateArticle } from "../../services/articlesAPI";
...
function EditArticleForm() {
  ...
  useEffect(
    function () {
      if (!isPending) {
        dispatch({ type: "label", payload: data.article.label });
        dispatch({ type: "namesl", payload: data.article.name.sl });
        dispatch({ type: "nameen", payload: data.article.name.en });
        dispatch({ type: "morning", payload: data.article.morning });

        dispatch({ type: "hiddenUsers", payload: data.article.hiddenUsers });
        dispatch({
          type: "hiddenReception",
          payload: data.article.hiddenReception,
        });
        dispatch({
          type: "visits",
          payload: data.article.visits,
        });
        dispatch({
          type: "duration",
          payload: data.article.duration,
        });
        dispatch({ type: "gift", payload: data.article.gift });

        dispatch({
          type: "priceDDV",
          payload: data.article.priceDDV.toString(),
        });
        dispatch({
          type: "taxRate",
          payload: (data.article.taxRate * 100).toString(),
        });

        if (data.article.type)
          dispatch({ type: "type", payload: data.article.type });

        if (data.article.activationDuration)
          dispatch({
            type: "activationDuration",
            payload: data.article.activationDuration,
          });

        if (data.article.noClasses) {
          dispatch({
            type: "noClasses",
            payload: data.article.noClasses,
          });
          dispatch({ type: "startDate", payload: data.article.startDate });
          dispatch({ type: "endDate", payload: data.article.endDate });
        }

        if (data.article.visits)
          dispatch({
            type: "visits",
            payload: data.article.visits,
          });

        if (data.article.duration)
          dispatch({
            type: "duration",
            payload: data.article.duration,
          });

        data.article.ageGroup.forEach((aG: string) =>
          dispatch({ type: "ageGroup", payload: aG }),
        );
      }
    },
    [isPending, data],
        ...
        await updateArticle(
          {
            ...
            startDate,
            endDate,
            price: (Number(priceDDV) / (Number(taxRate) + 1)).toFixed(2),
          },
          articleId,
        );
          ...
          {label === "VV" && (
            <div className="flex flex-col gap-1 justify-self-end">
              ...
              <div className="grid grid-cols-[64px_64px] rounded-lg border border-gray-300 bg-white px-2 py-1 shadow-xs">
                <button
                  className={`h-8 w-full cursor-pointer rounded-lg ${noClasses?.toString() === "1" ? "outline-secondary bg-primary/55 outline-2" : ""}`}
                  onClick={(e) => {
                    e.preventDefault();
                ...
                <button
                  className={`h-8 w-full cursor-pointer rounded-lg ${noClasses?.toString() === "2" ? "outline-secondary bg-primary/55 outline-2" : ""}`}
                  onClick={(e) => {
                    e.preventDefault();
    ...
    <div>
      ...
      <div className="flex items-center justify-between rounded-xl bg-white p-8">
        ...
        <input
          ...
          defaultValue={type === "terminska" ? duration : visits}
        />
      </div>
    </div>
  );
      ...
      <div className="flex items-center justify-between rounded-xl bg-white p-8">
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            value={(parseFloat(taxRate) * 100).toString()}
          />
        </div>
        <div className="flex flex-col gap-1">
